...
import pickle
from queryrewrite_data import get_loader
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        from queryrewrite_model import VLT5QueryRewrite, VLBartQueryRewrite

        model_kwargs = {}
        if 't5' in args.backbone:
            model_class = VLT5QueryRewrite
        elif 'bart' in args.backbone:
            model_class = VLBartQueryRewrite
        ...
        if self.verbose:
            ...
            if not self.wandb_initialized:
                ...
                if 't5' in self.args.backbone:
                    project_name = "VLT5_QueryRewrite"
                elif 'bart' in self.args.backbone:
                    project_name = "VLBart_QueryRewrite"
    ...
    def predict(self, loader, dump_path=None):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(loader, ncols=120, desc="Prediction", disable=not self.verbose)):
                ...
                questions.extend(batch['reference'])
                ids.extend(batch['img_id'])
                if 'target' in batch:
