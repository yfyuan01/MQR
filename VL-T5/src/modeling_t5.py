...
class VLT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config):
        ...
        self.prob_proj = nn.Linear(config.d_model, 1)
        self.init_weights()
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Add Pointer Copy Mechanism
        loss = None
        if pointer_gen:
            decode_attn = decoder_outputs[4][-1].sum(dim=1) # B, T, S
            memory = encoder_outputs.last_hidden_state # B, S, H
            decode_output = decoder_outputs[0] # B, T, H
            if input_ids is not None:
                src_full = input_ids.unsqueeze(1).repeat([1, decode_output.size(1), 1])   # B, T, S
            else:
                src_full = encoder_input_ids.unsqueeze(1).repeat([1, decode_output.size(1), 1])   # B, T, S
            context = torch.matmul(decode_attn, memory) # [B, T, H]
            prob = torch.sigmoid(self.prob_proj(context))  #B, T, H -> B, T, 1  
            gen_logits = prob * self.lm_head(decode_output)
#             gen_logits = prob * torch.softmax(self.lm_head(decode_output), dim=-1)                 # [B, T, V]
            copy_logits = torch.zeros_like(gen_logits)                                      # [B, T, V]
            copy_logits = copy_logits.scatter_add(2, src_full, decode_attn[:,:,:src_full.size(-1)])  # [B, T, V]
